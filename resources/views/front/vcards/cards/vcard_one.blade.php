...
@endsection
@section('page_css')
    <link rel="stylesheet" href="{{ mix('assets/css/web/vcards/vcards.css') }}" type="text/css"/>
@endsection
@section('content')
    <div class="container p-0">
        <div class="vcard-page-content position-relative">
            ...
            <div>
                <img src="{{ !empty($vcard->cover_image) ? $vcard->cover_image :  asset('assets/web/css/images/vcard-one.jpeg') }}"
                     class="vcard-page-banner bg_cover">
            </div>
            <div class="admin-wrapper d-flex justify-content-center">
                <div class="admin-card position-relative d-flex flex-sm-row flex-column p-0 align-items-center">
                    <div class="admin-card-image d-flex justify-content-sm-start justify-content-center m-sm-0 m-3">
                        <img class="thumb"
                             src="{{ !empty($vcard->profile_image) ? $vcard->profile_image : $data['user']->profile_image}}"
                             alt="Thumb"></div>
                    <div class="content p-3 w-100">
                        <div class="admin-title pb-3">
                            <h5 class="text  mb-2">{{ $vcard->name }}</h5>
                            <span class="admin-position">{{ $vcard->occupation }}</span>
                        </div>
                        <p class="text-muted text-center mt-2 pt-2 mb-0">
                            {!!  $vcard->introduction !!}
                        </p>
                    </div>
                </div>
