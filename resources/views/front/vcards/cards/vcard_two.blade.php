...
@endsection
@section('page_css')
    <link href="{{ asset('assets/css/web/vcards/vcards-two.css') }}" rel="stylesheet" type="text/css"/>
@endsection
@section('content')
    <div class="container p-0">
        <div class="vcard-page-content position-relative">
            ...
            <div>
                <img src="{{ !empty($vcard->cover_image) ? $vcard->cover_image :  asset('assets/web/css/images/vcardtwo.jpeg') }}"
                     class="vcard-page-banner vcard_bg_cover_two">
            </div>
            <div class="admin_wrapper_card d-flex justify-content-center">
                <div class="admin_card position-relative d-sm-flex">
                    <div class="d-flex align-items-center flex-column justify-content-center img_container pb-sm-0 pb-3 col-sm-6">
                        <div class="d-flex justify-content-center admin-card-image mx-3 mb-sm-0 mb-2">
                            <img class="rounded-circle vcard_card_thumb"
                                 src="{{ !empty($vcard->profile_image) ? $vcard->profile_image :  $data['user']->profile_image }}"
                                 alt="Thumb">
                        </div>
                        <div class="content mx-3">
                            <div class="admin-title">
                                <h5 class="text center my-2">{{ $vcard->name }}</h5>
                                <span class="admin-position">{{ $vcard->occupation }}</span>
                            </div>
                        </div>
                    </div>
                    <p class="text-muted text-center mb-0 d-flex align-items-center justify-content-center px-sm-3 px-0 pt-3 pb-sm-3 pb-0 col-sm-6">
                        {!!  $vcard->introduction !!}
                    </p>
                </div>
            </div>
