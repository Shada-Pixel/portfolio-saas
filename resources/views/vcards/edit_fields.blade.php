<div class="row">
        ...
        <div class="row">
            ...
            <div class="w-auto mt-2">
                <img id='profileImagePreview' class="img-thumbnail thumbnail-preview"
                     src="{{ !empty($vCards->profile_image) ? $vCards->profile_image : $user->profile_image }}">
            </div>
        </div>
    </div>
    <div class="form-group col-6">
        <div class="row">
            ...
            <div class="w-auto mt-2">
                <img id='coverImagePreview' class="img-thumbnail thumbnail-preview"
                     src="{{ isset($vCards->cover_image) ? $vCards->cover_image : asset('img/infyom-logo.png')}}">
            </div>
        </div>
    </div>
    ...
    <div class="col-sm-12 mt-3">
        <div class="mb-3 h5">
            {{__('messages.vCards.edit_v_card_attribute')}}
        </div>
        <table class="table table-bordered table-responsive-sm" id="vCardAttributeTbl">
            <thead class="">
            ...
            @foreach($vCards->vCardAttributes as $key => $vCardAttribute)
                <tr>
                    <td class="text-center item-number custom-number">{{$key+1}}</td>
                    <td class="text-center">
                        <button class="btn btn-primary button-icon-size mt-1 iconpicker dropdown-toggle editVCardAttribute"
                                data-iconset="fontawesome5"
                                data-icon="{{$vCardAttribute->icon}}" role="iconpicker" data-original-title="" title=""
                                aria-describedby="popover984402" data-id="{{$key+1}}">
                        </button>
                        <input class="form-control plan-icon edit-vcard-attribute-icon{{$key+1}}" name="icon[]"
                               type="hidden"
                               value="{{$vCardAttribute->icon}}">
                    </td>
                    <td class="text-center">
                        <div class="edit-color-wrapper"></div>
                        <input class="form-control color" id="edit_color" name="icon_color[]" hidden type="text"
                               value="{{$vCardAttribute->icon_color}}">
                    </td>
                    <td>
                        <input class="form-control" name="label_text[]"
                               type="text" value="{{ $vCardAttribute->label_text  }}" maxlength="25"
                               pattern="^\S[a-zA-Z ]+$" title="Attribute Label Not Allowed White Space"
                               placeholder="{{__('messages.vCards.vCards_placeholder.enter_label_name')}}">
                    </td>
                    <td>
                        <input class="form-control" name="value_text[]" maxlength="100"
                               value="{{ $vCardAttribute->value_text  }}"
                               type="text"
    ...
    <div class="form-group col-sm-12 mt-3 d-flex align-items-center">
        ...
        <a href="{{ route('vcards.index') }}" class="btn btn-light text-dark ml-1">{{__('messages.cancel')}}</a>
    </div>
</div>
